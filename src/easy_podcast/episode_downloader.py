...
class EpisodeDownloader:
    ...
    def download_episode(self, episode: Episode, target_path: str) -> DownloadResult:
        ...
        try:
            ...
            if file_path:
                ...
                return DownloadResult(
                    success=False,
                    error=f"Failed to download {episode.title}"
                )
                
        except Exception as e:
            self.logger.error("Download error for episode %s: %s", episode.id, e)
            return DownloadResult(
                success=False,
                error=str(e)
            )
    ...
    def download_multiple(
        ...
        for episode, target_path in downloads:
            ...
            if result.success:
                ...
                self.logger.error("Failed: %s - %s", episode.title, result.error)
